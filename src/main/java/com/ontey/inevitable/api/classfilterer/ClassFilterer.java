package com.ontey.inevitable.api.classfilterer;
...
public class ClassFilterer {
   ...
   public static Set<Class<?>> find(Class<?> jarLocationSource) {
      return find(jarLocationSource, clazz -> true);
   }
   ...
   public static Set<Class<?>> find(Class<?> source, Function<Class<?>, Boolean> requirements) {
      ...
      try(JarFile jar = getJar(source)) {
         ...
         Log.error("Internal Error (probably): " + e.getClass().getSimpleName() + " in the AnnotationFinder");
