package com.ontey.inevitable.api.brigadier.argument;
...
public class Arg {
    ...
    * @param ctx The {@link CommandContext} of your argument (available in the lambda of {@link ArgumentBuilder#executes(Command) executes})
    * @param options The {@link CommandOptions} that should be used for fetching the error message
    * @param action The action executed if the executor is a player
    */
   
   public static int requirePlayer(CommandContext<CommandSourceStack> ctx, CommandOptions options, Consumer<Player> action) {
      Entity entity = ctx.getSource().getExecutor();
      CommandSender sender = ctx.getSource().getSender();
      
      if(!(entity instanceof final Player player)) {
         sender.sendMessage(options.getMessage("messages.incapable-executor", "That entity can't run this command!"));
         return FAIL;
      }
